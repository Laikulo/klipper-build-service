...
async function reconfigure() {
  modal();
  launch_kconfig(
    get_selected_ver().getKConfigBundleUrl(),
    await window.current_config.arrayBuffer(),
  );
}
...
function modal(name) {
  ...
  if (window.active_modal) {
    document
      .getElementById("modal_" + window.active_modal)
...
function get_selected_ver() {
  return window.kconfig_project.revisions[
    document.getElementById("form_v3_version").selectedOptions[0].value
  ];
}
...
function run_menuconfig_v3(ev) {
  ...
  if (document.getElementById("form_v3_config_file").checked) {
    ...
    if (!file_chooser.value) {
      alert(
        "File mode selected, but no file uploaded. Please check input and try again!",
      );
      return false;
    }
    file_list = file_chooser;
  }
  let kconfig_bundle_url =
    window.kconfig_project.revisions[
      document.getElementById("form_v3_version").selectedOptions[0].value
    ].getKConfigBundleUrl();
...
function launch_kconfig(kconfig_bundle_url, conf_file) {
  if (conf_file) {
    if (obj_type(conf_file) === "ArrayBuffer") {
      ...
      obj_type(conf_file) === "HTMLInputElement" &&
      conf_file.type === "file" &&
      conf_file.value
    ) {
...
async function populate_versions(project, target_selector) {
  ...
  let new_opts = (await window.kconfig_project.getRevisions())
    .entries()
    .map((e) => {
...
window.kbs_init = function () {
  ...
  let populate_promise = populate_versions(
    "klipper",
    document.getElementById("form_v3_version"),
  );
